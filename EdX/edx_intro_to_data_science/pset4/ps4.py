...
def generate_models(x, y, degs):
    ...
    result_models=[]
    for i in degs:
        temp_model=np.polyfit(x,y,i)
        result_models.append(temp_model)
...
def evaluate_models_on_training(x, y, models):
    ...
    pylab.plot(x,y,'o',label="Actual values")
    pylab.plot(x,np.polyval(best_model,x),label="best model")
    pylab.xlabel("x data")
    pylab.ylabel("y data")
    title='Fit degree'+str(len(best_model)) + "\n" + 'R2='+str(max_r_sq)
    pylab.title(title)
...
y = []
for year in INTERVAL_1:
    y.append(np.mean(raw_data.get_daily_temp(city, month, day, year)))
models = generate_models(x, y, [1])
evaluate_models_on_training(x, y, models)
