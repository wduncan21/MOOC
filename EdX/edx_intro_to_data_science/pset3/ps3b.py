...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    for i in range(numTrials):
        ...
        for j in range(timesteps):    
            #patient.update()
            temp_result.append(patient.update())
        results.append(temp_result)
    result_final=list(map(list, zip(*results)))
    average_pop=[sum(x)/float(numTrials) for x in result_final]
    pylab.plot(average_pop, label="Total Virus Population")
...
class ResistantVirus(SimpleVirus):
    ...
    def reproduce(self, popDensity, activeDrugs):
        ...
        resist_status=len(activeDrugs)-sum(self.isResistantTo(i) for i in activeDrugs)
        if resist_status>0:
            raise NoChildException('this virus does not reproduce')
        elif self.maxBirthProb * (1 - popDensity)>=random.random():
            resistances={}
            for drug in activeDrugs:
                if self.getMutProb()>=random.random():
                    resistances[drug]=not self.getResistances()[drug]
                else:
                    resistances[drug]=self.getResistances()[drug]
